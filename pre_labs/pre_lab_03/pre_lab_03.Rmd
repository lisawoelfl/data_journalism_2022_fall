...
**Answer** There are parsing issues
...
**Answer** 1,640 problems - R is expecting data that looks like some sort of true/false, but actually, there are numbers
...
**Answer** No parsing errors this time
...
**Answer** district: number; precinct: character
```{r}
...
**Answer**: I'm getting an error message, "couldn't find function " %>% ""
...
**Answer** 12 registrations in January 1900, the first 20 results suggest that there are some errors in the data and some very old people registered
```{r}
yadkin_voters %>% 
...
**Answer** In class we talked about the confirmation postcards, I think that would be something to look into. How many people forget to confirm? What are the consequences?
...
**Answer** Errors in spelling ("Conowingoo"), capitalization in data and column names, first column starts with a numeric
...
**Answer** 
The function adds the letter x so that the column name doesn't start with a numeric.
...
**Answer** R can't summarize the amount, because the column is recognized as "characters", not numericals.
...
**Answer** Derrick Hamilton - one duplicate
...
**Answer** Only the first 5 digits appear in the row - all donors have the same ZIP code
...
**Answer** In the city column, the first letter is now capitalized - 2 misspellings remain
...
**Answer** 1 spelling error remains
...
**Answer** No more spelling errors!
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
