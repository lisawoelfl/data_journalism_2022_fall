...
Next, let's read in the precinct voting location data and make sure you clean up the column names -> remove column names with spaces

```{r}
glimpse(precinct_voting_locations)
```
...
```{r}
names(md_primary_precinct_dem) <- gsub(" ", "_", names(md_primary_precinct_dem))
glimpse(md_primary_precinct_dem)

names(md_primary_precinct_rep) <- gsub(" ", "_", names(md_primary_precinct_rep))
glimpse(md_primary_precinct_rep)
```



These two dataframes have a pair of columns in common - not necessarily with the same column names. Join the two dataframes on that unique combination that describes a precinct and the jurisdiction that contains it: county & county_code // election_district_precinct & 

```{r}
md_primary_precinct_both <- bind_rows(list(md_primary_precinct_dem, md_primary_precinct_rep))
```


```{r}
cleaned_md_primary_precinct_both <- md_primary_precinct_both %>%
  clean_names()

cleaned_md_primary_precinct_both
```


```{r}
chr_md_primary_precinct_both <- cleaned_md_primary_precinct_both %>% mutate(election_district_precinct = as.character(election_district_precinct))

glimpse(chr_md_primary_precinct_both)
```


```{r}
combined_location_results <- chr_md_primary_precinct_both %>% left_join(cleaned_precinct_voting_locations, by=c("election_district_precinct"="precinct_code","county"="county_code"))

combined_location_results
```
...
A1. PG has the most locations, 327, although Montgomery has the largest population, it's only place 3 with voting locations. I remember seeing population figures in our prelab 4.

```{r}
wes_aruna <- combined_location_results %>% 
...
Q2. Write code to produce a `votes` column which is the result of adding together `early_votes`, `election_night_votes`, `provisional_votes` and the mail-in votes, overwriting your dataframe so that it now has that `total_votes` column going forward. 

Then, separately write code to show each county and precinct and calculate the total votes in the Governor's race - bearing in mind that each precinct will have multiple rows in the dataframe, but you only want to see one row for each precinct. What is the name and voting location of the precinct that saw the most votes in the governor's race? 

A2. Prince George's County, Dr. Henry A. Wise, Jr High School had the most votes, 2396.

```{r}
# mutate adds votes for each candidate

total_votes <- combined_location_results %>% 
  mutate(total_votes = early_votes+election_night_votes+mail_in_ballot_1_votes+mail_in_ballot_2_votes+provisional_votes)

# sum adds votes for all candidates

gov_total_votes <- total_votes %>% 
  group_by(county_name, location_name, election_district_precinct, party) %>%
...
A3. 89 precincts have 0 votes in the governor's race. Prince George's County has the most precincts with zero votes. I would need to know how many people are eligible to vote in those precincts.
...
gov_total_votes %>% 
  group_by(party) %>% 
  summarise(mean_gov = mean(total_votes))
```

```{r}
gov_total_votes %>% 
  ungroup() %>% 
  summarise(mean_gov = mean(total_votes), median_gov = median(total_votes))
```
...
A4: Which precinct has the highest number of mail-in ballots? My answer: Prince George's County's precinct Largo Community Church has the most mail-in votes.


```{r}
total_votes %>% 
  group_by(county_name, location_name, election_district_precinct) %>%
  summarise(mail_votes = sum(mail_in_ballot_1_votes,mail_in_ballot_2_votes)) %>% 
  arrange(desc(mail_votes))
