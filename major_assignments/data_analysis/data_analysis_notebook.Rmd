...
# We cleaned the data in OpenRefine
...
The original data had 12855 rows and 5 columns. Our cleaned dataframe has 12855 rows and 7 columns (we added cleaned columns for organization name & employer). In the dataset for all registrations, we have variables for the registrant's name, their organization, their employer, the form id and the registration period.

Limitations: The dataframe is pretty basic with a small number of columns. That means we will have to cross-reference the website, if we want to know how much a lobbyist is making or how much an employer is spending on lobbying. The general dataset doesn't even include the registration matters. For our topical questions, we have to download new datasets for every registration matter we're interested in. Another limitation is that the data doesn't go back very far. The earliest data is from 2019, which means we can't look at long-term trends.
